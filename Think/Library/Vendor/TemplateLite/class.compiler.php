<?php
/*
 * Project:	template_lite, a smarter templates engine
...
class Template_Lite_Compiler extends Template_Lite {
	...
	var $php_extract_vars		=	true;	// Set this to false if you do not want the $this->_tpl variables to be extracted for use by PHP code inside the templates.
	...
	var $_require_stack		=	array();	// stores all files that are "required" inside of the templates
...
//		$this->_func_regexp = '[a-zA-Z_]\w*';
	...
	function _compile_file($file_contents)
	{
		...
		// gather all templates tags
		preg_match_all("!{$ldq}\s*(.*?)\s*{$rdq}!s", $file_contents, $_match);
		$tags = $_match[1];

		// put all of the non-templates tag text blocks into an array, using the templates tags as delimiters
		$text = preg_split("!{$ldq}.*?{$rdq}!s", $file_contents);

		// compile templates tags
		...
		// build the compiled templates by replacing and interleaving text blocks and compiled tags
