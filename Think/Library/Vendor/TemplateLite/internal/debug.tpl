...
{if isset($_templatelite_debug_output) and $_templatelite_debug_output eq "html"}
	...
	{foreach key=key value=templates from=$_debug_tpls}
		...
		<font color={if $_debug_tpls[$key].type eq "templates"}brown{elseif $_debug_tpls[$key].type eq "insert"}black{else}green{/if}>
...
<SCRIPT language=javascript>
	...
	{foreach key=key value=templates from=$_debug_tpls}
		...
		_templatelite_console.document.write("<font color={if $_debug_tpls[$key].type eq "templates"}brown{elseif $_debug_tpls[$key].type eq "insert"}black{else}green{/if}>");
	...
	{foreachelse}
		_templatelite_console.document.write("<tr bgcolor=#eeeeee><td colspan=2><tt><i>No templates assigned</i></tt></td></tr>	");
	{/foreach}
	_templatelite_console.document.write("<tr bgcolor=#cccccc><td colspan=2><b>Assigned templates variables:</b></td></tr>");
	{foreach key=key value=vars from=$_debug_keys}
		_templatelite_console.document.write("<tr bgcolor={if $key % 2}#eeeeee{else}#fafafa{/if}>");
	...
	{foreachelse}
		_templatelite_console.document.write("<tr bgcolor=#eeeeee><td colspan=2><tt><i>No templates variables assigned</i></tt></td></tr>");
	{/foreach}
	_templatelite_console.document.write("<tr bgcolor=#cccccc><td colspan=2><b>Assigned config file variables (outer templates scope):</b></td></tr>");
	{foreach key=key value=config_vars from=$_debug_config_keys}
		_templatelite_console.document.write("<tr bgcolor={if $key % 2}#eeeeee{else}#fafafa{/if}>");
